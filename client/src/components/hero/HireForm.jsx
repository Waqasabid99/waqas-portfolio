  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setSubmitMessage('Project request submitted successfully! Redirecting to Dashboard.');
        ...
        setSubmitMessage(response.data.message || 'Submission failed. Please try again.');
        setSubmitStatus('error');
      }
      ...
      <div className="max-w-screen-lg mx-auto bg-white p-8 rounded-lg shadow-md border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {!isLoggedin && (
              <>
                ...
                <input
                  ...
                  className="border px-4 py-2 rounded-md"
                />
                <p className="max-w-fit text-center text-sm text-[#6e7b8d]">
                  Already have an account?{' '}
                  <button onClick={handleLoginstate} className="text-[#1365ff] font-semibold hover:underline">
                    Login
                  </button>
                </p>
              </>
            ...
            {isLoggedin && (
              <>
                <input
                  ...
                  className="border px-4 py-2 rounded-md"
                />
                <p className="max-w-fit max-h-fit text-sm text-[#6e7b8d]">
                  Don't Have an account?{' '}
                  <button onClick={handleLoginstate} className="text-[#1365ff] font-semibold inline hover:underline">
                    Create one
                  </button>
                </p>
                <input
          ...
          <button
            ...
            className={`px-6 py-3 rounded-full border transition w-full md:w-auto ${isSubmitting
